...
docker save donorpipe-api donorpipe-nginx | ssh "$DPIPE_HOST" docker load

echo "==> Copying compose file(s) to host..."
ssh "$DPIPE_HOST" "mkdir -p $DPIPE_DIR"
scp docker-compose.yml "$DPIPE_HOST:$DPIPE_DIR/"

if [ "$PROD" = "1" ]; then
    scp docker-compose.prod.yml "$DPIPE_HOST:$DPIPE_DIR/"
    ssh "$DPIPE_HOST" "mkdir -p $DPIPE_DIR/nginx"
    scp nginx/nginx.prod.conf "$DPIPE_HOST:$DPIPE_DIR/nginx/nginx.prod.conf"
fi

echo "==> Starting containers on host..."
if [ "$PROD" = "1" ]; then
    ssh "$DPIPE_HOST" "cd $DPIPE_DIR && docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d"
else
    ssh "$DPIPE_HOST" "cd $DPIPE_DIR && docker compose up -d"
fi
